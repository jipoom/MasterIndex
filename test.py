...
def getTask(mode):
    ...
    if mode == "routine":
        ...
        taskCollection = retrieveCollection(MASTER_DB,MASTER_DB_PORT,'logsearch','service_config')
        #taskCollection = db.service_config
    elif mode == "error":
        taskCollection = retrieveCollection(MASTER_DB,MASTER_DB_PORT,'logsearch','indexer_state')
...
def retrieveCollection(dbHost,dbPort,dbName,colName):
    # Get last record from state DB
...
def checkIndexerState():
    ...
    #db = mongoClient.logsearch
    #IndexerStateCollection = db.indexer_state
    IndexerStateCollection = retrieveCollection(MASTER_DB,MASTER_DB_PORT,'logsearch','indexer_state')
...
def getIndexer(): 
    ...
    indexerCollection = retrieveCollection(MASTER_DB,MASTER_DB_PORT,'logsearch','MasterDB_indexer')
    indexerList = []
...
class ErrorRecoveryThread (threading.Thread):
    ...
    def run(self):
        ...
        indexerStateCollection = retrieveCollection(MASTER_DB,MASTER_DB_PORT,'logsearch','indexer_state')
        deadIndexer = indexerStateCollection.find({'state':'dead'})
        #stateDBCollection = getRecordFromStateDB()
        stateDBCollection = retrieveCollection(STATE_DB,STATE_DB_PORT,'logsearch','StateDB_state')
        indexerDetailCollection = retrieveCollection(MASTER_DB,MASTER_DB_PORT,'logsearch','MasterDB_indexer')
        #indexerDetailCollection = db.MasterDB_indexer
        #mongoClient.close()
        for i in range(0, deadIndexer.count()):
            ...
            if oldIndexer['state'] == "indexing":           
                ...
                try:
                    ...
                    # for unindexed records
                    changeState("update", oldIndexer['jobID'], "wait_indexing", "", "",oldIndexer['lastDoneRecord'])
                    # changeState("insert", oldIndexer['jobID'], "wait_writing", "",deadIdxIpAddr['ip_addr'],"0")
                    # changeState("update", oldIndexer['jobID'], "wait_indexing", "", "",oldIndexer['lastDoneRecord'])
